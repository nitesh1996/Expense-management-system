...
<tr class="info"><th>Date</th> <th> Loan </th><th> Money </th><th> Delete </th></tr>
<?php 
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc())
    {
        $exdate = strtotime($row["date"]);
        $exdate = date('d M Y', $exdate);
        echo "<tr><td> " . $exdate. "</td> <td> " . $row["loan"]. " </td><td> " . $row["lvalue"]. "</td> <td> <a href='homeedit.php?delloan=".$row['id']."' class='btn btn-danger btn-sm' name='remove' value='remove'><span class='glyphicon glyphicon-remove white' aria-hidden='true'></span></a></td></tr>";
...
<!-- Savings/Investment report table --->
<table class="table table-hover table-striped table-bordered ">
   <caption><h4><span class="glyphicon glyphicon-leaf" ></span> Savings/Investments of this Year</h4></caption>
<tr class="info"><th>Date</th> <th> Savings/Investments </th><th> Money </th><th> Delete </th></tr>
<?php 
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc())
    {
        $exdate = strtotime($row["date"]);
        $exdate = date('d M Y', $exdate);
        echo "<tr><td> " . $exdate. "</td> <td> " . $row["saving"]. " </td><td> " . $row["svalue"]. "</td> <td> <a href='homeedit.php?delsaving=".$row['id']."' class='btn btn-danger btn-sm' name='remove' value='remove'><span class='glyphicon glyphicon-remove white' aria-hidden='true'></span></a></td></tr>";
        $savingtotal+=$row["svalue"];
...
if(isset($_GET['delsaving']))
 {
     $id = (int)$_GET['delsaving'];
     $removeQuery = "UPDATE saving SET isdel='1' Where id=$id AND uid='$sid'";
