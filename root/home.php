...
if (charCode > 31 && (charCode != 46 &&(charCode < 48 || charCode > 57)))
    ...
    return true;
      ...
      <input type="text" class="form-control" size="20"  name="entrydate" required placeholder="Choose Date" id="datepicker3" readonly  aria-label="..." value="<?php	date_default_timezone_set("Asia/Kolkata");  $thisday = strtotime($today);
  ...
  <div class="input-group">
	  ...
	  <label><input type="radio" name="enttype"   value="3" aria-label="...">Loan</label>
	  <label><input type="radio" name="enttype"   value="4" aria-label="...">Savings</label>
      </span>
...
if ($conn->query($sql) === TRUE) {
	...
	elseif (isset($_POST["enttype"]) && trim($_POST["enttype"]) == "3") 
    {
$sql = "INSERT INTO loan (loan, lvalue, uid, date )
VALUES ('$edetail','$eamount','$uid','$entrydate')";
if ($conn->query($sql) === TRUE) {
	...
	elseif (isset($_POST["enttype"]) && trim($_POST["enttype"]) == "4") 
    {
$sql = "INSERT INTO saving (saving, svalue, uid, date )
VALUES ('$edetail','$eamount','$uid','$entrydate')";
if ($conn->query($sql) === TRUE) {
...
<!-- Loan report table --->
<table class="table table-hover table-striped table-bordered ">
   <caption><h4><span class="glyphicon glyphicon-leaf" ></span> Loan report for this Year</h4></caption>
<tr class="info"><th>Date</th> <th> Loan </th><th> Money </th><th><a href="homeedit.php" class='btn btn-sm'>Edit</a></th></tr>
<?php 
$sql = "SELECT * FROM loan WHERE uid='$sid' AND isdel=0 ORDER by date";
//$sql = "SELECT * FROM income WHERE date >= '$thiyear' AND date <= '$today' AND uid='$sid' AND isdel=0 ORDER by date";
$result = $conn->query($sql);
$loantotal = 0;
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc())
    {
        $exdate = strtotime($row["date"]);
        $exdate = date('d M Y', $exdate);
        echo "<tr><td> " . $exdate. "</td> <td> " . $row["loan"]. " </td><td> " . $row["lvalue"]. "</td> <td> <a href='homeedit.php' class='btn btn-sm' name='remove' value='remove'><span class='glyphicon glyphicon-remove white' aria-hidden='true'></span></a></td></tr>";
        $loantotal+=$row["lvalue"];

    }
} else {
}

echo "<tr id='totalLoan'><td> </td> <td> Total  </td><td>".$loantotal."</td></tr>";


if(isset($_GET['delincome']))
 {
     //$id = (int)$_GET['delincome'];
     //$removeQuery = "UPDATE loan SET isdel='1' Where id=$id AND uid='$sid'";
...
<!-- Loan report table --->
<table class="table table-hover table-striped table-bordered ">
   <caption><h4><span class="glyphicon glyphicon-leaf" ></span> Saving/Ivestment for this Year</h4></caption>
<tr class="info"><th>Date</th> <th> Saving/Ivestment </th><th> Money </th><th><a href="homeedit.php" class='btn btn-sm'>Edit</a></th></tr>
<?php 
$sql = "SELECT * FROM saving WHERE uid='$sid' AND isdel=0 ORDER by date";
//$sql = "SELECT * FROM income WHERE date >= '$thiyear' AND date <= '$today' AND uid='$sid' AND isdel=0 ORDER by date";
$result = $conn->query($sql);
$savingtotal = 0;
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc())
    {
        $exdate = strtotime($row["date"]);
        $exdate = date('d M Y', $exdate);
        echo "<tr><td> " . $exdate. "</td> <td> " . $row["saving"]. " </td><td> " . $row["svalue"]. "</td> <td> <a href='homeedit.php' class='btn btn-sm' name='remove' value='remove'><span class='glyphicon glyphicon-remove white' aria-hidden='true'></span></a></td></tr>";
        $savingtotal+=$row["lvalue"];

    }
} else {
}

echo "<tr id='totalsaving'><td> </td> <td> Total  </td><td>".$savingtotal."</td></tr>";


if(isset($_GET['delsaving']))
 {
     //$id = (int)$_GET['delincome'];
     //$removeQuery = "UPDATE loan SET isdel='1' Where id=$id AND uid='$sid'";
